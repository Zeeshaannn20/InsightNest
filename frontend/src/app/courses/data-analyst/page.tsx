...
export default function DataAnalystCoursePage() {
  ...
  const handleWaitlist = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      const entries = JSON.parse(localStorage.getItem("da_waitlist") || "[]");
      entries.push({ email: waitlistEmail, date: new Date().toISOString() });
      localStorage.setItem("da_waitlist", JSON.stringify(entries));
    }, 1000);
  };
          ...
          <div className="max-w-7xl mx-auto px-6">
            <div className="text-center mb-16">
              <div className="inline-flex items-center gap-2 bg-amber-100 text-amber-800 px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider mb-4">
                <span className="material-symbols-outlined text-sm">lock</span>
                Admissions Closed for Current Cohort
              </div>
              <h2 className="text-3xl md:text-4xl font-extrabold text-primary-container mb-4">Cohort Enrollment</h2>
            ...
            <div className="max-w-5xl mx-auto bg-surface-container-lowest p-8 md:p-12 rounded-3xl border-2 border-outline-variant/50 shadow-elevated relative overflow-hidden grid md:grid-cols-2 gap-12">
              <div className="absolute top-0 right-0 bg-amber-500 text-white px-6 py-1.5 rounded-bl-2xl text-xs font-bold uppercase tracking-wider">
                Admissions Closed
              </div>
              ...
              <div className="flex flex-col justify-between space-y-8">
                <div>
                  ...
                  <p className="text-sm text-outline mb-6 text-left">4-Week Live Cohort | AI-Powered Curriculum | Microsoft Fabric</p>
              ...
              {/* Right Column: Waitlist (replaces application form) */}
                ...
                {waitlistSubmitted ? (
                  <div className="text-center py-10 space-y-6 animate-scale-in">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-on-surface-variant leading-relaxed">
                        We will notify you at <strong>{waitlistEmail}</strong> as soon as the next cohort opens for enrollment.
                      </p>
                    </div>
                  </div>
                ) : (
                  <div className="space-y-6 text-left">
                    <div>
                      ...
                      <p className="text-xs text-on-surface-variant leading-relaxed">
                        Admissions for the current batch are closed. Enter your email below and we will notify you when the next cohort opens.
                      </p>
                    </div>
