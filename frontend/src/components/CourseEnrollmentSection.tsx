...
export default function CourseEnrollmentSection({ courseSlug }: CourseEnrollmentSectionProps) {
  ...
  // User Enrollment State (Optional Personalization for 'open' courses)
  ...
  useEffect(() => {
    async function fetchCourseAndEnrollment() {
      // 1. Fetch course details to know the status
      ...
      if (courseError || !courseData) {
        ...
        if (courseData.status === "open") {
          ...
          try {
            ...
            console.error("Failed to check user enrollment (optional personalization)", err);
            // Non-blocking, fallback to false
          }
  ...
  if (!course) {
    return (
      <section id="enrollment" className="py-24 bg-surface text-center">
        <p className="text-error">Failed to load course data.</p>
      </section>
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <h2 className="text-3xl md:text-4xl font-extrabold text-primary-container mb-4">
            {isComingSoon ? "Coming Soon" : isOpen ? "Start Your Journey Today" : "Cohort Enrollment"}
          </h2>
        ...
        <div className="max-w-5xl mx-auto bg-surface-container-lowest p-8 md:p-12 rounded-3xl border-2 border-outline-variant/50 shadow-elevated relative overflow-hidden grid md:grid-cols-2 gap-12">
          ...
          <div className="flex flex-col justify-between space-y-8">
            <div>
              ...
              <p className="text-sm text-outline mb-6 text-left">{course.duration_weeks ? `${course.duration_weeks}-Week Live Cohort | ` : ''}AI-Powered Curriculum</p>
              ...
              {isOpen && (
                <div className="bg-surface rounded-2xl p-6 border border-outline-variant/30 mb-6 text-center">
                  ...
                  <div className="flex items-center justify-center">
                    <span className="text-2xl line-through text-outline mr-3">&#x20B9;{originalPriceDisplay}</span>
          ...
          <div className="border-t md:border-t-0 md:border-l border-outline-variant/30 pt-8 md:pt-0 md:pl-12 flex flex-col justify-center">
            {isOpen ? (
              // Open State: Buy Button or Enrolled Message
              checkingEnrollment ? (
                <div className="flex justify-center py-8">
                  <span className="material-symbols-outlined animate-spin text-3xl text-secondary">progress_activity</span>
                </div>
              ) : isUserEnrolled ? (
                <div className="space-y-6 text-center bg-green-50 rounded-2xl p-8 border border-green-200">
                  ...
                  <div>
                    <h4 className="text-xl font-bold text-green-800 mb-2">You're already enrolled!</h4>
                    <p className="text-sm text-green-700 mb-6">
                      Head over to your student dashboard to access your course materials and live sessions.
                    </p>
                    <Link href="/dashboard" className="inline-block bg-green-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-green-700 transition-all shadow-md">
                      Go to Dashboard
                    </Link>
                  </div>
                </div>
              ...
              waitlistSubmitted ? (
                ...
                <div className="space-y-6 text-left">
                  <div>
                    ...
                    <h4 className="text-2xl font-bold text-primary-container mb-2">
                      {isComingSoon ? "Be the first to know" : "Join the Waitlist"}
                    </h4>
                    <p className="text-xs text-on-surface-variant leading-relaxed">
                      {isComingSoon 
                        ? "Enter your email below and we will notify you the moment enrollments open."
                        : "Admissions for the current batch are closed. Enter your email below to be notified when the next cohort opens."}
                    </p>
                  </div>
                  ...
                  <form onSubmit={handleWaitlist} className="space-y-4">
                    ...
                    >
                      {waitlistLoading ? (
                        ...
                        isComingSoon ? "Notify Me When Enrollments Open" : "Notify Me When Admissions Open"
                      )}
                    </button>
                  </form>
                </div>
